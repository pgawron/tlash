...
char* pc_str[N_PARAM_COMBOS] = { "n", "t",
                                 "c", "h" };
...
int main(int argc, char *argv[])
{
  ...
  for ( p = p_first, i = 1; p <= p_last; p += p_inc, i += 1 )
  {
    ...
    for ( param_combo = 0; param_combo < n_param_combos; param_combo++ ){
      ...
      fprintf( stdout, "data_copyt_%s( %d, 1:5 ) = [ %d  ", pc_str[param_combo], i, p );
      fflush( stdout );

      time_Copyt( param_combo, FLA_ALG_REFERENCE, n_repeats, m, n,
                 A, C, C_ref, &dtime, &diff, &gflops );

      fprintf( stdout, "%6.3lf %6.2le ", gflops, diff );
      fflush( stdout );

      time_Copyt( param_combo, FLA_ALG_FRONT, n_repeats, m, n,
                 A, C, C_ref, &dtime, &diff, &gflops );
  ...
  for ( i = 0; i < n_param_combos; i++ ) {
    fprintf( stdout, "plot( data_copyt_%s( :,1 ), data_copyt_%s( :, 2 ), '%c:%c' ); \n",
            pc_str[i], pc_str[i], colors[ i ], ticks[ i ] );
    fprintf( stdout, "plot( data_copyt_%s( :,1 ), data_copyt_%s( :, 4 ), '%c-.%c' ); \n",
            pc_str[i], pc_str[i], colors[ i ], ticks[ i ] );
  }
  ...
  for ( i = 0; i < n_param_combos; i++ )
    fprintf( stdout, "'ref\\_copyt\\_%s', 'fla\\_copyt\\_%s', ... \n", pc_str[i], pc_str[i] );
  ...
  fprintf( stdout, "title( 'FLAME copyt front-end performance (%s, %s)' );\n",
           m_dim_desc, n_dim_desc );
  fprintf( stdout, "print -depsc copyt_front_%s_%s.eps\n", m_dim_tag, n_dim_tag );
