...
FLA_Error FLA_Tevd_v_ops_var3r( int       m_A,
                                int       m_U,
                                int       n_CS,
...
FLA_Error FLA_Tevd_v_opd_var3r( int       m_A,
                                int       m_U,
                                int       n_CS,
                                ...
                                double*   buff_G, int rs_G, int cs_G )
{
	...
	for ( i = m_A - 1; i > 1; --i )
	{
		...
		{
			printf( "FLA_Tevd_v_opd_var3r: failed to converge (iteration %d)\n", i );
			FLA_Abort();
		}

#ifdef PRINTF
		printf( "FLA_Tevd_v_opd_var3r: found eig  %14.11f in col %3d (n=%d) after %3d iterations\n", buff_d[ r_val*inc_d ], r_val, m_ATL, k );
#endif

		// Apply the resulting Givens rotations to update the Givens matrix.
		//FLA_Apply_G_rf_opd_var1( k,
		FLA_Apply_G_rf_asd_var1a( k,
		                         m_U,
		                         m_ATL,
		                         buff_C, rs_C, cs_C,
		                         buff_S, rs_S, cs_S,
		...
		{
#ifdef PRINTF
			printf( "FLA_Tevd_v_opd_var3r: Internal deflation in col %d, eig %d\n", r_val, i );
			printf( "FLA_Tevd_v_opd_var3r: alpha11         = %23.19e\n", buff_d[r_val*inc_d] );
			printf( "FLA_Tevd_v_opd_var3r: alpha21 alpha22 = %23.19e %23.19e\n", buff_e[r_val*inc_e], buff_d[(r_val+1)*inc_d] );
#endif

			//printf( "FLA_Tevd_v_opd_var3r: iter so far = %3d\n", k_total );
	...
	//printf( "FLA_Tevd_v_opd_var3r: total iter  = %3d\n", k_total );
...
#ifdef PRINTF
		printf( "FLA_Tevd_v_opd_var3r: found eig  %14.11f in col %3d (n=%d) after %3d iterations (via Hevv)\n", buff_d[ 1*inc_d ], 1, 2, 1 );
