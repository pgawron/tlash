...
void time_LU_piv(
               ...
               FLA_Obj A, FLA_Obj b, FLA_Obj b_orig, FLA_Obj ipiv, FLA_Obj norm, 
...
int main(int argc, char *argv[])
{
  ...
  for ( p = p_first, i = 1; p <= p_last; p += p_inc, i += 1 )
  {
    ...
    FLA_Obj_create( FLA_INT, min_m_n, 1, 0, 0, &ipiv );
...
/*
    time_LU_piv( 0, FLA_ALG_REFERENCE, n_repeats, p, nb_alg,
                 A, b, b_orig, ipiv, norm, &dtime, &diff, &gflops );

    fprintf( stdout, "data_REF( %d, 1:2 ) = [ %d  %6.3lf ]; \n", i, p, gflops );
    fflush( stdout );
*/

    for ( variant = 3; variant <= 5; variant++ ){
      
      fprintf( stdout, "data_var%d( %d, 1:7 ) = [ %d  ", variant, i, p );
      fflush( stdout );

      time_LU_piv( variant, FLA_ALG_UNBLOCKED, n_repeats, p, nb_alg,
                   A, b, b_orig, ipiv, norm, &dtime, &diff, &gflops );

      fprintf( stdout, "%6.3lf %6.2le ", gflops, diff );
      fflush( stdout );

      time_LU_piv( variant, FLA_ALG_UNB_OPT, n_repeats, p, nb_alg,
                   A, b, b_orig, ipiv, norm, &dtime, &diff, &gflops );

      fprintf( stdout, "%6.3lf %6.2le ", gflops, diff );
      fflush( stdout );

      time_LU_piv( variant, FLA_ALG_BLOCKED, n_repeats, p, nb_alg,
                   A, b, b_orig, ipiv, norm, &dtime, &diff, &gflops );
