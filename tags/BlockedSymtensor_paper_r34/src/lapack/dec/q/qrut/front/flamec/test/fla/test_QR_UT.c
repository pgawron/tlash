...
int main(int argc, char *argv[])
{
  ...
  for ( p = p_first, i = 1; p <= p_last; p += p_inc, i += 1 )
  {
    ...
    for ( variant = 0; variant < n_variants; variant++ ){
      ...
      fprintf( stdout, "data_qr_ut( %d, 1:3 ) = [ %d  ", i, p );
      fflush( stdout );
/*
      time_QR_UT( variant, FLA_ALG_REFERENCE, n_repeats, m, n,
	              A, A_ref, t, T, W, b, b_ref, &dtime, &diff, &gflops );

      fprintf( stdout, "%6.3lf %6.2le ", gflops, diff );
      fflush( stdout );
*/
      time_QR_UT( variant, FLA_ALG_FRONT, n_repeats, m, n,
	              A, A_ref, t, T, W, b, b_ref, &dtime, &diff, &gflops );
...
/*
  ...
  fprintf( stdout, "title( 'FLAME QR_UT front-end performance (%s, %s)' );\n", 
