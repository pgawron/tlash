...
int main(int argc, char *argv[])
{
  ...
  for ( p = p_first, i = 1; p <= p_last; p += p_inc, i += 1 )
  {
    ...
    for ( variant = 0; variant < n_variants; variant++ ){
      ...
      fprintf( stdout, "data_lq_ut( %d, 1:3 ) = [ %d  ", i, p );
      fflush( stdout );

      time_LQ_UT( variant, FLA_ALG_REFERENCE, n_repeats, m, n,
	              A, A_ref, t, T, W, b, b_ref, &dtime, &diff, &gflops );

      fprintf( stdout, "%6.3lf %6.2le ", gflops, diff );
      fflush( stdout );

      time_LQ_UT( variant, FLA_ALG_FRONT, n_repeats, m, n,
	              A, A_ref, t, T, W, b, b_ref, &dtime, &diff, &gflops );
...
/*
  ...
  for ( i = 0; i < n_variants; i++ ) {
    fprintf( stdout, "plot( data_lq_ut( :,1 ), data_lq_ut( :, 2 ), '%c:%c' ); \n",
            colors[ i ], ticks[ i ] );
    fprintf( stdout, "plot( data_lq_ut( :,1 ), data_lq_ut( :, 4 ), '%c-.%c' ); \n",
            colors[ i ], ticks[ i ] );
  }
  ...
  for ( i = 0; i < n_variants; i++ )
    fprintf( stdout, "'ref\\_lq\\_ut', 'fla\\_lq\\_ut', ... \n" );
  ...
  fprintf( stdout, "title( 'FLAME LQ_UT front-end performance (%s, %s)' );\n", 
           m_dim_desc, n_dim_desc );
  fprintf( stdout, "print -depsc lq_ut_front_%s_%s.eps\n", m_dim_tag, n_dim_tag );
