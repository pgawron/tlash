...
/*
    time_Ttmm_u( 0, FLA_ALG_REFERENCE, n_repeats, p, nb_alg,
...
*/
    ...
    for ( variant = 1; variant <= n_variants; variant++ ){
      ...
      time_Ttmm_u( variant, FLA_ALG_UNBLOCKED, n_repeats, p, nb_alg,
                   A, x, b, norm, &dtime, &diff, &gflops );

      fprintf( stdout, "%6.3lf %6.2le ", gflops, diff );
      fflush( stdout );

      time_Ttmm_u( variant, FLA_ALG_UNB_OPT, n_repeats, p, nb_alg,
                   A, x, b, norm, &dtime, &diff, &gflops );

      fprintf( stdout, "%6.3lf %6.2le ", gflops, diff );
      fflush( stdout );

      time_Ttmm_u( variant, FLA_ALG_BLOCKED, n_repeats, p, nb_alg,
                   A, x, b, norm, &dtime, &diff, &gflops );
...
/*
  ...
  fprintf( stdout, "title( 'FLAME ttmm\\_u performance (%s)' );\n", 
           m_dim_desc );
  fprintf( stdout, "print -depsc ttmm_u_%s.eps\n", m_dim_tag );
