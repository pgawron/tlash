...
int main(int argc, char *argv[])
{
  ...
  for ( p = p_first, i = 1; p <= p_last; p += p_inc, i += 1 )
  {
    ...
    for ( param_combo = 0; param_combo < n_param_combos; param_combo++ ){
      ...
      fprintf( stdout, "data_scal_%s( %d, 1:5 ) = [ %d  ", pc_str[param_combo], i, p );
      fflush( stdout );

      time_Scal( param_combo, FLA_ALG_REFERENCE, n_repeats, m, n,
                 A, C, C_ref, &dtime, &diff, &gflops );

      fprintf( stdout, "%6.3lf %6.2le ", gflops, diff );
      fflush( stdout );

      time_Scal( param_combo, FLA_ALG_FRONT, n_repeats, m, n,
                 A, C, C_ref, &dtime, &diff, &gflops );
  ...
  for ( i = 0; i < n_param_combos; i++ ) {
    fprintf( stdout, "plot( data_scal_%s( :,1 ), data_scal_%s( :, 2 ), '%c:%c' ); \n",
            pc_str[i], pc_str[i], colors[ i ], ticks[ i ] );
    fprintf( stdout, "plot( data_scal_%s( :,1 ), data_scal_%s( :, 4 ), '%c-.%c' ); \n",
            pc_str[i], pc_str[i], colors[ i ], ticks[ i ] );
  }
  ...
  for ( i = 0; i < n_param_combos; i++ )
    fprintf( stdout, "'ref\\_scal\\_%s', 'fla\\_scal\\_%s', ... \n", pc_str[i], pc_str[i] );
