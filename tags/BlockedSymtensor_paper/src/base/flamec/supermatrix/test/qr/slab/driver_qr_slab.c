...
{ 
   ...
   for ( i = 0; i < n_trials; i++ )
   {
      ...
      for ( j = 0; j < n_repeats; j++ )
      {
         ...
         FLASH_QR_UT_create_hier_matrices( A, 1, &nb_alg, &AH, &TW );
         FLASH_Obj_create_hier_copy_of_flat( x, 1, &nb_alg, &xH );
         ...
         FLASH_QR_UT_solve( AH, TW, bH, xH );

         FLASH_Obj_free( &AH );
         FLASH_Obj_free( &TW );
         
         FLASH_Obj_flatten( xH, x );
         FLASH_Obj_free( &xH );
         FLASH_Obj_free( &bH );
      }
