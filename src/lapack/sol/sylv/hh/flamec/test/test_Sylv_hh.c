...
int main(int argc, char *argv[])
{
  ...
  for ( p = p_first, i = 1; p <= p_last; p += p_inc, i += 1 )
  {
    ...
    time_Sylv_hh( 0, FLA_ALG_REFERENCE, n_repeats, m, n, nb_alg,
    ...
    for ( variant = 1; variant <= n_variants; variant++ ){
      ...
      time_Sylv_hh( variant, FLA_ALG_UNB_OPT, n_repeats, m, n, nb_alg,
                    isgn, A, B, C, C_ref, scale, &dtime, &diff, &gflops );

      fprintf( stdout, "%6.3lf %6.2le ", gflops, diff );
      fflush( stdout );

      time_Sylv_hh( variant, FLA_ALG_BLOCKED, n_repeats, m, n, nb_alg,
                    isgn, A, B, C, C_ref, scale, &dtime, &diff, &gflops );
  ...
  fprintf( stdout, "title( 'FLAME sylv\\_tt performance (%s)' );\n", 
           m_dim_desc );
  fprintf( stdout, "print -depsc sylv_tt_%s.eps\n", m_dim_tag );
