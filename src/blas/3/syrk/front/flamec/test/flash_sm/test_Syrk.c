...
char* pc_str[N_PARAM_COMBOS] = { "ln", "lt",
                                 "un", "ut" };
...
int main(int argc, char *argv[])
{
  ...
  for ( p = p_first, i = 1; p <= p_last; p += p_inc, i += 1 )
  {
    ...
    for ( param_combo = 0; param_combo < n_param_combos; param_combo++ ){
      ...
      fprintf( stdout, "data_syrk_%s( %d, 1:5 ) = [ %d  ", pc_str[param_combo], i, p );
      fflush( stdout );

      time_Syrk( param_combo, FLA_ALG_REFERENCE, n_repeats, m, k,
                 A, B, C, C_ref, &dtime, &diff, &gflops );

      fprintf( stdout, "%6.3lf %6.2le ", gflops, diff );
      fflush( stdout );

      time_Syrk( param_combo, FLA_ALG_FRONT, n_repeats, m, k,
                 A, B, C, C_ref, &dtime, &diff, &gflops );
...
/*
  ...
  for ( i = 0; i < n_param_combos; i++ ) {
    fprintf( stdout, "plot( data_syrk_%s( :,1 ), data_syrk_%s( :, 2 ), '%c:%c' ); \n",
            pc_str[i], pc_str[i], colors[ i ], ticks[ i ] );
    fprintf( stdout, "plot( data_syrk_%s( :,1 ), data_syrk_%s( :, 4 ), '%c-.%c' ); \n",
            pc_str[i], pc_str[i], colors[ i ], ticks[ i ] );
  }
  ...
  for ( i = 0; i < n_param_combos; i++ )
    fprintf( stdout, "'ref\\_syrk\\_%s', 'fla\\_syrk\\_%s', ... \n", pc_str[i], pc_str[i] );
  ...
  fprintf( stdout, "title( 'FLAME syrk front-end performance (%s, %s)' );\n",
           m_dim_desc, k_dim_desc );
  fprintf( stdout, "print -depsc syrk_front_%s_%s.eps\n", m_dim_tag, k_dim_tag );
