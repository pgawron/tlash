...
int main(int argc, char *argv[])
{
  ...
  for ( p = p_first, i = 1; p <= p_last; p += p_inc, i += 1 )
  {
    ...
    for ( j = 0; j < n_thread_experiments; j++ ){
      ...
      time_Syrk_ln( variant, FLA_ALG_OPENMP_2LOOPS, n_repeats, p, nb_alg,
                    A, B, C, C_ref, &dtime, &diff, &gflops );
  ...
  fprintf( stdout, "title( 'OpenFLAME syrk\\_ln\\_var%d (two loops) performance (%s, %s)' );\n", 
           variant, m_dim_desc, k_dim_desc );
  fprintf( stdout, "print -depsc syrk_ln_omp2l_var%d_%s_%s.eps\n", variant, m_dim_tag, k_dim_tag );
